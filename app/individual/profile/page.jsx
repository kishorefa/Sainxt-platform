...
export default function ProfileBuilder() {
    ...
    const checkAuth = async () => {
      try {
        ...
        const response = await fetch(
          "http://192.168.0.229:5000/api/read_users_me",
          {
            headers: {
  ...
  const handleMarketReadinessConfirm = async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.0.229:5000/api/ai-review/generate-scores",
        {
          method: "POST",
          headers: {
  ...
  const submitProfileToBackend = async () => {
    try {
      ...
      const profileResponse = await fetch(
        "http://192.168.0.229:5000/api/save_profile",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const educationResponse = await fetch(
        "http://192.168.0.229:5000/api/save_education",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (formData.workExperiences.length > 0) {
        ...
        const experienceResponse = await fetch(
          "http://192.168.0.229:5000/api/save_experience",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      if (formData.projects.length > 0) {
        ...
        const projectResponse = await fetch(
          "http://192.168.0.229:5000/api/save_project",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      const skillsResponse = await fetch(
        "http://192.168.0.229:5000/api/save_skills",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const preferencesResponse = await fetch(
        "http://192.168.0.229:5000/api/save_preferences",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                Detailed skill development pathway based on your profile
                analysis
              </p>
            </div>
            <Button
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Based on your profile analysis, here's your personalized skill
                development roadmap
              </CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="overflow-x-auto w-full lg:w-2/3">
                  <table className="w-full border-collapse border border-gray-200 dark:border-gray-700">
                    ...
                    <tbody>
                      {/* Individual Skills from Pathway as Table Rows */}
                      {aiResults?.evaluation?.skill_pathway?.length > 0 &&
                        aiResults.evaluation.skill_pathway
                          .filter((s) => typeof s === "string") // keep only simple skill strings
                          .map((skill, index) => {
                            // Extract just the heading part (before the colon)
                            const skillHeading = skill.includes(":")
                              ? skill.split(":")[0].trim()
                              : skill;

                            // Function to determine skill level and status color
                            const getSkillStatus = (skillName) => {
                              const skillText = skill.toLowerCase();

                              // Check for indicators in the skill description
                              if (
                                skillText.includes("basic") ||
                                skillText.includes("beginner") ||
                                skillText.includes("fundamental") ||
                                skillText.includes("start") ||
                                skillText.includes("improve") ||
                                skillText.includes("focus on improving")
                              ) {
                                return {
                                  level: "Low",
                                  color: "bg-red-500",
                                ...
                                skillText.includes("advanced") ||
                                skillText.includes("expert") ||
                                skillText.includes("proficient") ||
                                skillText.includes("strong") ||
                                skillText.includes("excellent") ||
                                skillText.includes("good")
                              ) {
                                return {
                                  level: "High",
                                  color: "bg-green-500",
                                ...
                                // Default to moderate if no clear indicator
                                return {
                                  level: "Moderate",
                                  color: "bg-yellow-500",
                                  hoverColor:
                            ...
                            return (
                              <tr
                                key={`skill-${index}`}
                                className={skillStatus.hoverColor}
                              >
                                <td className="border border-gray-200 dark:border-gray-700 px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">
                                  {skillHeading}
                                </td>
                                <td className="border border-gray-200 dark:border-gray-700 px-4 py-3 text-center">
                                  <div
                                    className={`w-4 h-4 ${skillStatus.color} rounded-full mx-auto`}
                                    title={skillStatus.level}
                                  ></div>
                                </td>
                      ...
                      {aiResults?.evaluation?.recommendations?.length > 0 && (
                        <tr className="hover:bg-blue-50 dark:hover:bg-blue-900/10">
                          ...
                          <td className="border border-gray-200 dark:border-gray-700 px-4 py-3 text-center text-gray-500 dark:text-gray-400">
                            -
                          </td>
                          <td className="border border-gray-200 dark:border-gray-700 px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                            <ul className="space-y-2">
