...
import { Sparkles, BrainCircuit, GraduationCap, BookOpen, Target, Award, Video, Check, X, Trophy, Star } from "lucide-react";
import { AITrainingVideo } from "@/components/training/AITrainingVideo";
...
export default function AITrainingSessionPage() {
  ...
  const handleVideoComplete = () => {
    ...
    // Don't auto-progress to next video
    // The user needs to complete the assignment first
  };
  ...
  const startMCQ = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/mcqs/start-assignment?category=${category}`);
      if (!response.ok) {
      ...
      {showSuccess && (
        <SuccessAnimation 
          onContinue={() => {
            ...
            if (currentVideoIndex === videoLessons.length - 1) {
              window.location.href = '/individual/certificates/ai-training-certificate';
            } else {
        ...
        <div className="flex h-[calc(100vh-64px)]">
          ...
          <div className="w-80 border-r border-gray-200 bg-white overflow-y-auto">
            <div className="p-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-800">AI Training Session</h2>
              <p className="text-sm text-gray-500 mt-1">5 lessons • 37 minutes • Interactive Learning</p>
            </div>
            <div className="p-2">
            ...
            <div className="max-w-4xl mx-auto">
              <div className="mb-6">
                ...
                <div className="mt-2 flex items-center text-sm text-gray-500">
                  <span>Lesson {currentVideoIndex + 1} of {videoLessons.length}</span>
              ...
              {!showMCQ ? (
                <div className="bg-white rounded-xl shadow-lg overflow-hidden" ref={videoRef}>
                  <AITrainingVideo
                    ref={videoRef}
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                  {score ? (
                    <div className="text-center py-8">
                      <div className={`text-4xl font-bold mb-4 ${score.score === score.total ? 'text-green-600' : 'text-red-600'}`}>
                      ...
                      <div className="mt-6 space-y-4 text-left max-w-2xl mx-auto">
                        {score.result.map((item, index) => (
                          <div key={index} className={`p-4 rounded-lg ${item.is_correct ? 'bg-green-50' : 'bg-red-50'}`}>
                            <div className="font-medium">Q: {item.question}</div>
                            <div className="mt-1 text-sm">
                              <span className="font-medium">Your answer:</span> {item.your_answer}
                              {!item.is_correct && (
                                <span className="ml-2">
                                  <span className="font-medium">(Correct answer: {item.correct_answer})</span>
                                </span>
                              )}
                            </div>
                          </div>
              ...
              <div className="mt-6 flex justify-between items-center">
                ...
                <div className="flex space-x-3">
                  ...
                  {isVideoWatched && !completedVideos.has(currentVideoIndex) && !showMCQ ? (
                    <Button 
                      onClick={startMCQ}
                      variant="default"
                      className="bg-green-600 hover:bg-green-700"
                  ...
                  ) : currentVideoIndex < videoLessons.length - 1 && !showMCQ ? (
                    <Button 
                      onClick={() => handleVideoSelect(currentVideoIndex + 1)}
                      variant="default"
                    >
                    ...
                    <Button 
                      onClick={() => {
                        ...
                        window.location.href = '/individual/assessments/ai-training-assessment';
                      }}
                      variant="default"
                      className="bg-green-600 hover:bg-green-700"
                    >
              ...
              <div className="mt-8 bg-white rounded-xl shadow p-6">
                ...
                <p className="text-gray-700">
                  {currentVideo.description} This lesson is part of our comprehensive AI Training Series 
                  designed to give you a solid foundation in artificial intelligence concepts and applications.
                </p>
                
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-3">Lesson Content</h3>
